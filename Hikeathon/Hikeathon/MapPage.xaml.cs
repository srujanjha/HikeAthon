...
namespace Hikeathon
{
    public sealed partial class MapPage : Page
    {
        ...
        private void OnTapped(MapControl sender, MapInputEventArgs args)
        {
            try
            {
                var elementList = HikeathonMap.FindMapElementsAtOffset(args.Position);
        ...
        private async void OnHomeClicked(object sender, Windows.UI.Xaml.RoutedEventArgs e)
        {
            ...
            if (_app.Hikeathon != null)
            {
                foreach (Place place in _app.Hikeathon)
                {
                    if (place.Kind == PlaceKind.Home)
                    {
                        HikeathonMap.Center = new Geopoint(place.Position);
                        HikeathonMap.ZoomLevel = 16;
                        found = true;
                        break;
                    }
                }
        ...
        {
            HikeathonMap.Center = _currentLocation;
            HikeathonMap.ZoomLevel = 16;
        }
        ...
        private void OnFrequentClicked(object sender, Windows.UI.Xaml.RoutedEventArgs e)
        {
            ...
            HikeathonMap.Center = new Geopoint(notHomeNorWork.ElementAt(_chosenFrequentId).Position);
            HikeathonMap.ZoomLevel = 16;
        }
