...
namespace Hikeathon
{
    ...
    {
        // Constants
        private readonly Color HikeathonColorHome = Color.FromArgb(AlphaLevelOfHikeathonCircle, 31, 127, 31);
        private readonly Color HikeathonColorWork = Color.FromArgb(AlphaLevelOfHikeathonCircle, 255, 163, 31);
        private readonly Color HikeathonColorFrequent = Color.FromArgb(AlphaLevelOfHikeathonCircle, 127, 127, 255);
        private readonly Color HikeathonColorKnown = Color.FromArgb(AlphaLevelOfHikeathonCircle, 255, 127, 127);
        private readonly Color HikeathonColorUnknown = Color.FromArgb(AlphaLevelOfHikeathonCircle, 127, 127, 127);
        private const int AlphaLevelOfHikeathonCircle = 127;
        ...
        private Hikeathon.App _app = Application.Current as Hikeathon.App;
            ...
            if (_monitor == null)
            {
                ...
                if (await Monitor.IsSupportedAsync())
                {
                    ...
                    if (await CallSensorcoreApiAsync(async () => { _monitor = await Monitor.GetDefaultAsync(); }))
                    {
                        ...
                        // Update list of known Hikeathon
                        await UpdateKnownHikeathonAsync();
        ...
        private async void EnterFence(GeofenceMonitor sender, object e)
        {
            ...
            await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () =>
            {
                foreach (GeofenceStateChangeReport report in reports)
                {
                    ...
                    {
                        foreach (Place place in _app.Hikeathon)
                        {
        ...
        private void CreateCircle(BasicGeoposition position, double radius, Color circleColor)
        {
            ...
            HikeathonMap.MapElements.Add(circle);
        }
        ...
        /// <summary>
        /// Updates list of known Hikeathon
        /// </summary>
        /// <returns>Asynchronous task</returns>
        private async Task UpdateKnownHikeathonAsync()
        {
            if (_monitor != null)
            {
                _app.Hikeathon = null;

                HikeathonMap.MapElements.Clear();
    
                if (await CallSensorcoreApiAsync(async () => { _app.Hikeathon = await _monitor.GetKnownPlacesAsync();}))
                {
                    foreach (var p in _app.Hikeathon)
                    {
                        ...
                        {
                            case PlaceKind.Home:
                                color = HikeathonColorHome;
                                break;
                            case PlaceKind.Work:
                                color = HikeathonColorWork;
                                break;
                            case PlaceKind.Frequent:
                                color = HikeathonColorFrequent;
                                break;
                            case PlaceKind.Known:
                                color = HikeathonColorKnown;
                                break;
                            default:
                                color = HikeathonColorUnknown;
                                break;
                        }

                        HikeathonMap.MapElements.Add(icon);
                        CreateGeofence(p.Id.ToString(), p.Position.Latitude, p.Position.Longitude, p.Radius, color);
                    }
                }
            }
            Debug.WriteLine("Known Hikeathon updated.");
        }
